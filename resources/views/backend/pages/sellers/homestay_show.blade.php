...
@section('contents')
    <div class="content-wrapper">
        ...
        <section class="content">
                    ...
                    <div class="row">
                        <div class="col-12 col-sm-6">
                            <div class="col-12 top-box-image">
                                @if ($homestay->images && $homestay->images->isNotEmpty())
                            ...
                            <div class="col-12 position-relative">
                                ...
                                <div class="product-image-thumbs d-flex flex-nowrap overflow-hidden">
                                    @forelse ($homestay->images ?? [] as $index => $image)
                                        <div
                        ...
                        <div class="col-12 col-sm-6">
                            ...
                            <div class="bg-gray py-2 px-3 mt-4">
                                ...
                                <h5 class="mt-0">
                                    <small>Price (Between Check In to Check Out Time): {{ $homestay->price }} </small>
                                </h5>
                            </div>
                        ...
                        <div class="col-12 col-lg-6">
                            ...
                            <div class="tab-content p-3" id="nav-tabContent">
                                <div class="tab-pane fade show active" id="product-basic-detail" role="tabpanel"
                                    ...
                                    <h6>Food Available: <b>{{ $homestay->food_allowed }}</b></h6>
                                    <h6 class="mt-3">Room Type: <b>{{ $homestay->room_type }}</b></h6>
                                    <h6 class="mt-3">Bedroom Type: <b>{{ $homestay->bedroom_type }}</b></h6>
                                    <h6 class="mt-3">Number of Bedroom: <b>{{ $homestay->number_of_rooms }}</b></h6>
                                    <h6 class="mt-3">Number of Single Rooms:
                                        <b>{{ $homestay->number_of_single_rooms }}</b>
                                    </h6>
                                    <h6 class="mt-3">Number of Double Rooms:
                                        <b>{{ $homestay->number_of_double_rooms }}</b>
                                    </h6>
                                </div>
                                <div class="tab-pane fade" id="product-address-detail" role="tabpanel"
                                    ...
                                    <h6 class="mt-3">State: <b>{{ $homestay->state->name }}</b></h6>
                                    <h6 class="mt-3">District: <b>{{ $homestay->district->name }}</b></h6>
                                    <h6 class="mt-3">City: <b>{{ $homestay->city }}</b></h6>
                                    <h6 class="mt-3">Address: <b>{{ $homestay->address }}</b></h6>
                                    <h6 class="mt-3">Pin Code: <b>{{ $homestay->pincode }}</b></h6>
                                </div>
                                <div class="tab-pane fade" id="product-capacity" role="tabpanel"
                                    aria-labelledby="product-capacity-tab">

                                    <h6>Number of Adult: <b>{{ $homestay->number_of_adults }}</b></h6>
                                    <h6 class="mt-3">Number of Children: <b>{{ $homestay->number_of_children }}</b></h6>
                                    <h6 class="mt-3">
                                        Check In Time:
                                        <b>
                                            {{ date('d-M-Y', strtotime($homestay->check_in_time)) }}
                                        </b>
                                    </h6>
                                    <h6 class="mt-3">
                                        Check Out Time:
                                        <b>
                                            {{ date('d-M-Y', strtotime($homestay->check_out_time)) }}
                                        </b>
                                    </h6>
                                    <h6 class="mt-3">Area (sq.ft): <b>{{ $homestay->area }}</b></h6>
                                    <h6 class="mt-3">Guest Access: <b>{{ $homestay->guest }}</b></h6>
                                    <h6 class="mt-3">Mountain View: <b>{{ $homestay->mountain_view }}</b></h6>
                                </div>
                            </div>
                        </div>
                        <div class="col-12 col-lg-6">
                            <figure class="d-flex justify-content-end align-items-center">
                                <img src="{{ asset('storage/' . $homestay->room_image) }}" alt="{{ $homestay->name }}"
                                    class="img-fluid room-image">
                            </figure>
                        </div>
                    </div>
                    {{-- {{ $homestay->commonSpaces }} --}}
                    <div class="row mt-4">
                        <nav class="w-100">
                            <div class="nav nav-tabs" id="product-tab" role="tablist">
                                {{-- @if ($homestay->benefits && $homestay->benefits->isNotEmpty()) --}}
                                ...
                                {{-- @endif --}}
                                {{-- @if ($homestay->commonSpaces && $homestay->commonSpaces->isNotEmpty()) --}}
                                ...
                                {{-- @endif --}}
                                {{-- @if ($homestay->safetySecurities && $homestay->safetySecurities->isNotEmpty()) --}}
                                <a class="nav-item nav-link" id="product-safety-security-tab" data-toggle="tab"
                                    ...
                                    Security</a>
                                {{-- @endif --}}
                                {{-- @if ($homestay->beddings && $homestay->beddings->isNotEmpty()) --}}
                        ...
                        <div class="tab-content p-3" id="nav-tabContent">
                            ...
                            <div class="tab-pane fade" id="product-common-space" role="tabpanel"
                                ...
                                <ul>
                                    @forelse ($homestay->commonSpaces as $common)
                            ...
                            <div class="tab-pane fade" id="product-safety-security" role="tabpanel"
                                ...
                                <ul>
                                    @forelse ($homestay->safetySecurities as $safe)
                            ...
                            <div class="tab-pane fade" id="product-bedding" role="tabpanel"
                                ...
                                <ul>
                                    @forelse ($homestay->beddings as $bed)
                            ...
                            <div class="tab-pane fade" id="product-rules-policies" role="tabpanel"
                                ...
                                <ul>
                                    <li>Up to 3 days Prior: <b>{{ $homestay->upto_3days_prior }}</b></li>
                                    <li class="mt-3">Up to 2 days Prior: <b>{{ $homestay->upto_2days_prior }}</b></li>
                                    <li class="mt-3">Up to 1 days Prior: <b>{{ $homestay->{"1day_prior"} }}</b></li>
                                    <li class="mt-3">On the day of check-in:
                                        <b>{{ $homestay->same_day_cancellation }}</b>
                                    </li>
                                    <li class="mt-3">No show: <b>{{ $homestay->no_show }}</b></li>
                                </ul>
                            </div>
                        </div>
                    </div>
            ...
            @if (count($homestay->registerUsers) > 0)
                <div class="card card-solid mt-4">
                    <div class="card-body">
                        <table id="registeredUsersList" class="table table-bordered table-striped">
                            ...
                            <tbody>
                                @forelse ($homestay->registerUsers as $index => $user)
                                    <tr>
                                        <td>{{ $index + 1 }}</td>
                                        <td>{{ $user['name'] }}</td>
                                        <td>{{ $user['phone'] }}</td>
